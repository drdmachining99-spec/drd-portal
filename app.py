...
def normalize_phone(raw):
    """Normalizes a phone number down to its last 10 digits (a full local mobile
    number) so two numbers only match when they are genuinely the same number —
    not just because they happen to share a short digit suffix."""
...
def make_whatsapp_link(req, settings):
    ...
    # Only mention payments/balance once money has actually moved, or the order is
    # Completed. A brand-new quotation with nothing paid yet should just show the
    # price — no "balance due" pressure before the client has even agreed to it.
    if advance_paid > 0 or status == 'Completed':
...
def build_document_pdf(req, document_type='quotation'):
    ...
    # Payment status block — only relevant once money has actually moved, or on a
    # final Invoice. A fresh Quotation with nothing paid yet should just show the
    # price, not talk about balances/pending payment before the client has agreed.
    advance_paid = safe_float(req.get('advance_paid'))
    balance_due = safe_float(req.get('balance_due'), max(0.0, net - advance_paid))
    show_payment_block = (document_type == 'invoice') or advance_paid > 0
    if show_payment_block:
        if y - 40 < bottom:
            footer(page_no)
            c.showPage()
            page_no += 1
...
{% if req.advance_paid and req.advance_paid > 0 or req.status == 'Completed' %}<p style="font-size:14px;background:#f8f9fa;border-radius:8px;padding:10px 12px"><b>Payments Received:</b> {{ money(req.advance_paid) }}{% if req.balance_due and req.balance_due > 0 %}<br><b style="color:#b02a37">Balance Due (Pending):</b> {{ money(req.balance_due) }}{% else %}<br><b style="color:#198754">Fully Paid ✅</b>{% endif %}</p>{% endif %}
...
{% if o.status != 'New' %}<span style="font-size:13px">Total: <b>{{money(o.net_payable)}}</b>{% if o.advance_paid and o.advance_paid > 0 or o.status == 'Completed' %} | Paid: <b>{{money(o.advance_paid)}}</b>{% if o.balance_due and o.balance_due > 0 %} | <b style="color:#b02a37">Balance: {{money(o.balance_due)}}</b>{% else %} | <b style="color:#198754">Fully Paid</b>{% endif %}{% endif %}</span><br>
...
{% if o.status == 'Completed' %}{% if o.balance_due and o.balance_due > 0 %}<a class="btnlink" style="background:#e6a100" href="/payment/{{o.job_id}}">⏳ Pay Balance to Unlock Invoice</a>{% else %}<a class="btnlink" style="background:#198754" href="/invoice/{{o.job_id}}.pdf" target="_blank">Invoice</a>{% endif %}{% endif %}
<a class="btnlink" style="background:#6c757d" href="/track/{{o.job_id}}">Track</a>
</div>{% endfor %}
<a class="plain" href="/">← Submit a new order</a></div></body></html>'''

INVOICE_LOCKED_PAGE = '''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(160deg,#0d2b40,#123f5d 40%,#1d6fa5);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:16px;margin:0}.box{max-width:460px;background:white;padding:36px 30px;border-radius:16px;box-shadow:0 10px 35px rgba(0,0,0,.25);text-align:center}.box h2{color:#e6a100;margin-top:0}.amt{background:#fff8ec;border:1.5px dashed #e6a100;border-radius:8px;padding:12px;font-size:20px;font-weight:bold;color:#b06a00;margin:14px 0}a.btnlink{display:block;padding:13px;border-radius:8px;color:white;text-decoration:none;font-weight:bold;margin-top:12px;background:linear-gradient(135deg,#123f5d,#1d6fa5)}a.plain{display:block;margin-top:14px;color:#1d6fa5;text-decoration:none;font-size:14px}</style></head><body><div class="box"><h2>⏳ Invoice Not Yet Available</h2><p>Your order <b>{{req.job_id}}</b> is complete, but the invoice unlocks once the remaining balance is received.</p><div class="amt">Balance Due: {{ money(req.balance_due) }}</div><a class="btnlink" href="/payment/{{req.job_id}}">💳 Pay the Balance Now</a><a class="plain" href="/track/{{req.job_id}}">← Back to order status</a></div></body></html>'''
...
def my_orders():
    ...
    Uses an EXACT normalized-number match (not a loose suffix match) so one
...
def verify_payment(job_id):
    ...
    is still in Quotation Sent (advance) or already Completed (balance payment)."""
    all_reqs = load_requests()
    result_status = 'Quotation Sent'
    for req in all_reqs:
        if req.get('job_id') != job_id:
            continue
        ...
        if terms == '100_advance':
            ...
            if req['balance_due'] <= 0:
                ...
                req['balance_payment_status'] = 'Verified'
        result_status = req.get('status', 'Quotation Sent')
        break
    save_all_requests(all_reqs)
    tab = 'done' if result_status == 'Completed' else 'progress'
    msg = f'Payment verified for {job_id}'
    return redirect(url_for('admin_dashboard', msg=msg, tab=tab, job=job_id))
...
def complete_job(job_id):
    all_reqs = load_requests()
    blocked_msg = None
    completed_with_balance = False
    for req in all_reqs:
        if req.get('job_id') != job_id:
            continue
        terms = req.get('payment_terms')
        balance_due = safe_float(req.get('balance_due'))
        if terms == '100_advance':
            # Full payment must be in before this order can be marked complete at all.
            if balance_due > 0.01:
                blocked_msg = f'Cannot complete {job_id} — full (100%) payment must be verified first.'
                break
        else:
            # 50/50 or custom terms: at least the advance/first payment must be verified
            # before production/completion can be marked. The balance can still be pending.
            if req.get('advance_payment_status') != 'Verified' and safe_float(req.get('advance_paid')) <= 0:
                blocked_msg = f'Cannot complete {job_id} — the advance payment must be verified first.'
                break
        ...
        completed_with_balance = balance_due > 0.01
        break
    if blocked_msg:
        return redirect(url_for('admin_dashboard', msg=blocked_msg, tab='progress', job=job_id))
    save_all_requests(all_reqs)
    if completed_with_balance:
        msg = f'Order {job_id} marked Completed — balance payment is still pending, invoice will unlock once it is paid.'
    else:
        msg = f'Order {job_id} marked Completed — invoice generated'
    return redirect(url_for('admin_dashboard', msg=msg, tab='done', job=job_id))
...
def invoice_pdf(job_id):
    ...
    if not req:
        return 'Job not found', 404
    if safe_float(req.get('balance_due')) > 0.01:
        return render_template_string(INVOICE_LOCKED_PAGE, req=req, money=money), 402
    try:
